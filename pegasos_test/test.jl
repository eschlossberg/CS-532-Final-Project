...
pred_vecs = reshape(pred_vecs,10000 ,784 )
...
function polynomial(x::Array{Float64}, y::Array{Float64})
	return (dot(x,y) + 1)^2
end

alpha = pegasos(X,y, 0.005, 4000, RBF)
alpha_poly = pegasos(X, y, 0.05, 2000, polynomial)
w_2 = pegasos(X,y, 0.1,500 )
println("making predictions")
preds = SharedArray{Float64}(size(pred_vecs,1))
@parallel for idx in 1:size(pred_vecs, 1)
